...
} from 'angular-permission-control-build-tools';
...
/** Path to the output of the Angular permission-control package. */
const angularPermissionControlOutPath = join(outputDir, 'packages', '@angular-permission-control');

/** Array of vendors that are required to serve the demo-app. */
const appVendors = [
  '@angular', 'systemjs', 'zone.js', 'rxjs', 'core-js'
];
...
task(':watch:devapp', () => {
  ...
  watch(join(angularPermissionControlOutPath, '**/*.css'), [':build:devapp:scss', triggerLivereload]);
});
...
task('serve:devapp', ['build:devapp'], sequenceTask(
  [':serve:devapp', 'angular-permission-control:watch', ':watch:devapp']
));
...
task('stage-deploy:devapp', ['build:devapp'], () => {
  ...
  copyFiles(angularPermissionControlOutPath, '**/prebuilt/*.+(css|map)', join(outDir, 'dist/packages/angular-permission-control'));
});
